...
class persona:
    ...
    from datetime import datetime #VERLO CON GONZA
    @staticmethod
    def check_fecha_de_nacimiento(fechadenacimiento):
        while fechadenacimiento > datetime.today():
            print('la fecha ingresada no es valida')
            fechadenacimiento=input("ingrese la fecha nuevamente:")
...
class empleado(persona):
    ...
    @staticmethod
    def checklegajo(legajo, matriz_empleado):
        while legajo in matriz_empleado[:6] or legajo.isnumeric()==False or len(legajo)!=4:
            if legajo in matriz_empleado[:6]:
